...
public class SLinkedList<E extends DeepClone<E>> implements Iterable<E>
{
	...
	//Setting size to zero. Method not really needed, but I wanted to do it this way (sorry about that).
	...
	{ 
		return (size == 0); //Will return true if the size is equal to zero
	}
	...
	/*
 	...
 	public SLinkedList<E> deepClone()
 	{
 		...
 		//Created a new SLinkedList
 		SLinkedList<E> myDeepCopy = new SLinkedList<E>();
 		
 		//Setting a temporary node as my head
 		SNode<E> myTemp = head;
 		int i = 0;
 		 
 		while (myTemp != null)
 		{
 			//Adding elements using the deepClone method (as suggested)
 			myDeepCopy.add(i, myTemp.element.deepClone());
 			myTemp = myTemp.next;
 			i++;
 		}
	...
	/*
	 *     @param  index of node to get (Line 296)
	...
	{    //  I have to use a different generic type since I am defining a class here.
		...
		// T is the generice type here.
		//T is user defined, and we are basically saying that every SNode SHOULD HAVE IT.
